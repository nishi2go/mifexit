...
public class FTPHandler extends FileHandler
{
    ...
    static List<RouterPropsInfo> properties = new ArrayList<RouterPropsInfo>(
	    FileHandler.getFileProperties());
    ...
    private boolean createDirectoryStructure(FTPClient ftp, String[] dirNameList)
	    throws IOException
    {
	if (dirNameList.length > 0)
	{
		...
		return createDirectoryStructure(ftp,
			(String[]) Arrays.copyOfRange(dirNameList, 1,
				dirNameList.length));
	    }
	    else
	    {
		if (ftp.makeDirectory(dirName))
		{
		    return ftp.changeWorkingDirectory(dirName)
			    && createDirectoryStructure(ftp,
				    (String[]) Arrays.copyOfRange(dirNameList,
					    1, dirNameList.length));
		}
		else
		{
		    return false;
    ...
    {
	final String blValue = getPropertyValue(FTPConstants.NOTCPDELAY
		.toString());
    ...
    {
	final String bufSize = getPropertyValue(FTPConstants.BUFFERSIZE
		.toString());
	...
	try
	{
	    return InetAddress.getByName(getPropertyValue(FTPConstants.HOSTNAME
		    .toString()));
	}
    ...
    {
	final MaxEndPointPropInfo fileDir = endPointPropVals
		.get(MicConstants.FILEDIR);
	...
	// available.
	endPointPropVals.put(MicConstants.FILEDIR, new MaxEndPointPropInfo(
		MicConstants.FILEDIR, "." + File.separator, 1));
